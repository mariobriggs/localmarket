Activities = new CouchDB.Database('activities');
if (Meteor.isServer) {
  Activities._ensureIndex({date: 'desc'});
  Activities._ensureIndex([{recipeName: 'desc'},{date: 'desc'}]);
}
...
Activities.latest = function() {
  return Activities.find({date: {$gt: '0'}}, {sort: {date: 'desc'}, limit: 1});
}
